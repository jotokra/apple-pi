...
FAILED=0
if [[ "$target" == "all" ]]; then
	for s in sanitize structure onboard-sandbox vault-roundtrip vault-tracefree vault-telemetry-safe vault-rotate-import-export vault-onboarding vault-masked-overlay vault-export-to vault-failclosed docs-build; do run_one "$s"; done
else
	run_one "$target"
fi
