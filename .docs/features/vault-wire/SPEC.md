...
`agent-secrets` store (Hermes/Claude, plaintext), and per-bridge `process.env`
...
"vault": {
  ...
  "wire": {
    "zai":      { "to": "auth" },                                   // → auth.json (provider key, plaintext in P1; !command in P3)
    "minimax":  { "to": "auth" },
...
```jsonc
"zai": { "type": "api_key", "key": "!<vault-reader> zai" }
```
