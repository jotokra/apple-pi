...
## 4. Hermes leftover analysis — keep / redesign / drop

| Hermes piece | Verdict | Why |
|---|---|---|
...
| Boards JSON (`~/.hermes/kanban/boards/`) | **DROP** | Third representation; pure duplication |
...
| `kanban-bridge.ts` (pi read-only bridge to Hermes DB) | **REPLACE** | With MD-aware read/write tools (M9) |
| Hermes `tasks`-as-execution-queue | **REDESIGN** → the `sess_*`+`analysis_*` durable tier replaces "agent memory"; execution (if ever) is a separate reader | Keep memory, drop the fleet |

**Net:** ~80% of Hermes surface goes away; what remains is cards + a fast
