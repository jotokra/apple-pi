...
| M11 | Migration (cards + 89 sessions + autoresearch; decommission Hermes) | 4 | M10 | yes |
...
### M11-4 · Decommission Hermes kanban (gated, last) ⬜  *(confirm with user first)*
- **What:** stop the `kanban-roadmap-sync` cron; archive `~/.hermes/kanban/boards/`;
  unset pi `KANBAN_DB_PATH` (was the Hermes DB); remove `kanban-bridge.ts`
...
- **REQ-M11-4:** new tools the only kanban surface; web kanban either repointed
  or formally retired.
- **Verify:** `apple-pi kanban list` works with no Hermes DB.
